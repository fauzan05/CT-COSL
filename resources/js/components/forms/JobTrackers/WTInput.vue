...
const completeSelection = computed(() => {
    if (selectedWT.value && selectedUnit.value) {
        return {
            size: selectedWT.value,
            unit: selectedUnit.value,
            label: `${selectedWT.value} ${selectedUnit.value}`
        }
    }
    return {
        size: '',
...
const addNewOption = async () => {
    if (newOptionSize.value.trim()) {
        try {
            await axios.post(`${baseUrl}/api/job-tracker-master/wts`, {
                size: newOptionSize.value.trim()
            })

            await fetchAllWTs() // Refresh the list options from the API

            toast.success('WT option added successfully!')

            newOptionSize.value = ''
            showAddOption.value = false
        } catch (error) {
            console.error('Error adding WT option:', error)
            toast.error('Failed to add WT option.')
        }
    } else {
...
const updateOption = async ({ index, oldValue, newValue }) => {
    // Extract WT name from newValue object or use it directly if it's a string
    const newWTName = typeof newValue === 'object' ? newValue.size : newValue

    try {
        await axios.put(`${baseUrl}/api/job-tracker-master/wts/${sizeOptions.value[index].id}`, {
            size: newWTName.trim()
        })

        await fetchAllWTs() // Refresh the list options from the API

        toast.success('WT option updated successfully!')
    } catch (error) {
        console.error('Error updating WT option:', error)
        toast.error('Failed to update WT option.')
    }
}

const removeOption = async (index) => {
    try {
        await axios.delete(`${baseUrl}/api/job-tracker-master/wts/${sizeOptions.value[index].id}`)

        await fetchAllWTs() // Refresh the list options from the API

        toast.success('WT option removed successfully!')
    } catch (error) {
        console.error('Error removing WT option:', error)
        toast.error('Failed to remove WT option.')
    }
}

const fetchAllWTs = async () => {
    try {
        const response = await axios.get(`${baseUrl}/api/job-tracker-master/wts`)
