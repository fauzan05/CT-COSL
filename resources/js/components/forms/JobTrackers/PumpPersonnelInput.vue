    ...
    <div>
        ...
        <div class="flex items-center justify-between mb-2">
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                Pump Personnel ({{ pumpPersonnels.length }})
            </label>
        ...
        <div class="space-y-2 w-auto">
            <div v-for="(personnel, index) in pumpPersonnels" :key="`personnel-${index}`" class="flex items-center gap-2 w-auto">
                <input 
                    ...
                    :placeholder="`Enter Pump Personnel Name ${index + 1}`" 
                    @input="handleInput(index, $event)" 
                />

                <!-- Remove button (only show if more than 1 input) -->
                <button 
                    v-if="pumpPersonnels.length > 1" 
                    @click="removePumpPersonnel(index)" 
                    type="button"
...
// Reactive array untuk menyimpan nilai-nilai Pump Personnel
const pumpPersonnels = ref([])
const isInternalUpdate = ref(false)

// Initialize dengan props
const initializePumpPersonnels = () => {
    if (props.modelValue && props.modelValue.length > 0) {
        pumpPersonnels.value = props.modelValue.map(item => {
        ...
        pumpPersonnels.value = [{ value: '' }]
    }
}

// Initialize on mount
initializePumpPersonnels()
...
const addNewPersonnel = () => {
    pumpPersonnels.value.push({ value: '' })
    emitValues()
}

// Function untuk menghapus input tertentu
const removePumpPersonnel = (index) => {
    if (pumpPersonnels.value.length > 1) {
        pumpPersonnels.value.splice(index, 1)
        emitValues()
    }
}
...
const handleInput = (index, event) => {
    pumpPersonnels.value[index].value = event.target.value
    emitValues()
}
...
const emitValues = () => {
    ...
    const allPersonnels = pumpPersonnels.value.map(personnel => ({
        value: personnel.value || ''
    }))
    
    emit('update:modelValue', allPersonnels)
}
</script>