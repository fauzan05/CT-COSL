    ...
    <div class="mb-5">
        <div class="flex justify-between items-center mb-2">
            ...
            <div class="flex items-center gap-2">
                <!-- reset button -->
                <button @click="selectedCJInjector = ''" type="button"
        ...
        <Listbox v-model="selectedCJInjector">
            <div class="relative">
                <ListboxButton
                    ...
                    <span class="block truncate text-gray-900 dark:text-white">
                        {{ selectedCJInjector || placeholder }}
                    </span>
        ...
        <div v-if="showAddOption" class="mt-2 flex gap-2">
            <input v-model="newOption" type="text" placeholder="Enter new CJ Injector"
...
const placeholder = ref('Select CJ Injector')
const listOptions = ref([])

const selectedCJInjector = ref('')
...
const addNewOption = async () => {
    if (newOption.value.trim()) {
        try {
            await axios.post(`${baseUrl}/api/job-tracker-master/cj-injectors`, {
                cj_injector_name: newOption.value.trim()
            })

            fetchAllCJInjectors() // Refresh the list options from the API

            toast.success('CJ Injector option added successfully!')

            newOption.value = ''
            showAddOption.value = false
        } catch (error) {
            console.error('Error adding CJ Injector option:', error)
            toast.error('Failed to add CJ Injector option.')
        }
    }
}
...
const updateOption = async ({ index, oldValue, newValue }) => {
    // Extract CJ Injector name from newValue object or use it directly if it's a string
    const newCJInjectorName = typeof newValue === 'object' ? newValue.cj_injector_name : newValue

    try {
        await axios.put(`${baseUrl}/api/job-tracker-master/cj-injectors/${listOptions.value[index].id}`, {
            cj_injector_name: newCJInjectorName.trim()
        })

        fetchAllCJInjectors() // Refresh the list options from the API

        toast.success('CJ Injector option updated successfully!')
    } catch (error) {
        console.error('Error updating CJ Injector option:', error)
        toast.error('Failed to update CJ Injector option.')
    }
}

const removeOption = async (index) => {
    try {
        await axios.delete(`${baseUrl}/api/job-tracker-master/cj-injectors/${listOptions.value[index].id}`)

        fetchAllCJInjectors() // Refresh the list options from the API

        toast.success('CJ Injector option removed successfully!')
    } catch (error) {
        console.error('Error removing CJ Injector option:', error)
        toast.error('Failed to remove CJ Injector option.')
    }
}

const fetchAllCJInjectors = async () => {
    try {
        const response = await axios.get(`${baseUrl}/api/job-tracker-master/cj-injectors`)
        if (response.data && Array.isArray(response.data)) {
            listOptions.value = [] // Reset options before populating
            // Sort by cj_injector_name alphabetically
            response.data.sort((a, b) => a.cj_injector_name.localeCompare(b.cj_injector_name))

            listOptions.value = response.data.map(loc => ({
                id: loc.id,
                value: loc.cj_injector_name,
                label: loc.cj_injector_name
            }))

            if (listOptions.value.length < 1 || !listOptions.value.some(opt => opt.value === selectedCJInjector.value)) {
                selectedCJInjector.value = ''
            }
        }
    } catch (error) {
