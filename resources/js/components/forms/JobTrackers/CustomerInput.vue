...
const addNewOption = async () => {
    if (newOption.value.trim()) {
        try {
            const response = await axios.post(`${baseUrl}/api/job-tracker-master/customers`, {
                customer_name: newOption.value.trim()
            })
            
            // Add to listOptions with the returned ID
            const newCustomer = {
                id: response.data.id,
                value: newOption.value.trim(),
                label: newOption.value.trim()
            }
            listOptions.value.push(newCustomer)
            
            // Sort listOptions alphabetically
            listOptions.value.sort((a, b) => a.label.localeCompare(b.label))
            
            console.log(`Customer option "${newOption.value}" added successfully!`)
...
const removeOption = async (index) => {
    ...
    try {
        ...
        const updatedCustomers = props.modelValue.filter(cust => cust !== optionToRemove)
        emit('update:modelValue', updatedCustomers)
    ...
    try {
        ...
        if (response.data && Array.isArray(response.data)) {
            ...
            listOptions.value = response.data.map(cust => ({
                id: cust.id,
                value: cust.customer_name, // Fix: was custome_name (typo)
