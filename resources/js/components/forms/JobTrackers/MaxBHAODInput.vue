    ...
    <div class="mb-5">
        ...
        <div class="grid grid-cols-3 gap-2">
            ...
            <div class="col-span-2">
                <Listbox v-model="selectedMaxBHAODSize">
                    <div class="relative">
                        <ListboxButton
                            ...
                            <span class="block truncate text-gray-900 dark:text-white">
                                {{ selectedMaxBHAODSize || sizePlaceholder }}
                            </span>
...
const selectedMaxBHAODSize = ref('')
...
const completeSelection = computed(() => {
    if (selectedMaxBHAODSize.value && selectedUnit.value) {
        return {
            size: selectedMaxBHAODSize.value,
            unit: selectedUnit.value,
            label: `${selectedMaxBHAODSize.value} ${selectedUnit.value}`
        }
    }
    return {
        size: '',
...
const addNewOption = async () => {
    if (newOptionSize.value.trim()) {
        try {
            await axios.post(`${baseUrl}/api/job-tracker-master/max-bha-ods`, {
                size: newOptionSize.value.trim()
            })

            await fetchAllMaxBHAODSizes() // Refresh the list options from the API

            toast.success('MaxBHAOD option added successfully!')

            newOptionSize.value = ''
            showAddOption.value = false
        } catch (error) {
            console.error('Error adding completion option:', error)
            toast.error('Failed to add completion option.')
        }
    } else {
...
const updateOption = async ({ index, oldValue, newValue }) => {
    // Extract completion name from newValue object or use it directly if it's a string
    const newMaxBHAODSizeName = typeof newValue === 'object' ? newValue.size : newValue

    try {
        await axios.put(`${baseUrl}/api/job-tracker-master/max-bha-ods/${sizeOptions.value[index].id}`, {
            size: newMaxBHAODSizeName.trim()
        })

        await fetchAllMaxBHAODSizes() // Refresh the list options from the API

        toast.success('MaxBHAOD option updated successfully!')
    } catch (error) {
        console.error('Error updating completion option:', error)
        toast.error('Failed to update completion option.')
    }
}

const removeOption = async (index) => {
    try {
        await axios.delete(`${baseUrl}/api/job-tracker-master/max-bha-ods/${sizeOptions.value[index].id}`)

        await fetchAllMaxBHAODSizes() // Refresh the list options from the API

        toast.success('MaxBHAOD option removed successfully!')
    } catch (error) {
        console.error('Error removing completion option:', error)
        toast.error('Failed to remove completion option.')
    }
}

const fetchAllMaxBHAODSizes = async () => {
    try {
        const response = await axios.get(`${baseUrl}/api/job-tracker-master/max-bha-ods`)
        if (response.data && Array.isArray(response.data)) {
            ...
            if (sizeOptions.value.length < 1 || !sizeOptions.value.some(opt => opt.value === selectedMaxBHAODSize.value)) {
                selectedMaxBHAODSize.value = ''
            }
        }
    } catch (error) {
...
watch(() => props.modelValue, (newValue) => {
    if (newValue?.size !== selectedMaxBHAODSize.value) {
        selectedMaxBHAODSize.value = newValue?.size || ''
    }
