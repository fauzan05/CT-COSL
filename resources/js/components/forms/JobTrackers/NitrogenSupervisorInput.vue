    ...
    <div class="mb-5">
        <div class="flex justify-between items-center mb-2">
            ...
            <div class="flex items-center gap-2">
                <!-- reset button -->
                <button @click="selectedNitrogenSupervisor = ''" type="button"
        ...
        <Listbox v-model="selectedNitrogenSupervisor">
            <div class="relative">
                <ListboxButton
                    ...
                    <span class="block truncate text-gray-900 dark:text-white">
                        {{ selectedNitrogenSupervisor || placeholder }}
                    </span>
        ...
        <div v-if="showAddOption" class="mt-2 flex gap-2">
            <input v-model="newOption" type="text" placeholder="Enter new Nitrogen Supervisor"
...
const placeholder = ref('Select Nitrogen Supervisor')
const listOptions = ref([])

const selectedNitrogenSupervisor = ref('')
...
const addNewOption = async () => {
    if (newOption.value.trim()) {
        try {
            await axios.post(`${baseUrl}/api/job-tracker-master/nitrogen-supervisors`, {
                nitrogen_supervisor_name: newOption.value.trim()
            })

            fetchAllNitrogenSupervisors() // Refresh the list options from the API

            toast.success('Nitrogen Supervisor option added successfully!')

            newOption.value = ''
            showAddOption.value = false
        } catch (error) {
            console.error('Error adding Nitrogen Supervisor option:', error)
            toast.error('Failed to add Nitrogen Supervisor option.')
        }
    }
}
...
const updateOption = async ({ index, oldValue, newValue }) => {
    // Extract Nitrogen Supervisor name from newValue object or use it directly if it's a string
    const newNitrogenSupervisorName = typeof newValue === 'object' ? newValue.nitrogen_supervisor_name : newValue

    try {
        await axios.put(`${baseUrl}/api/job-tracker-master/nitrogen-supervisors/${listOptions.value[index].id}`, {
            nitrogen_supervisor_name: newNitrogenSupervisorName.trim()
        })

        fetchAllNitrogenSupervisors() // Refresh the list options from the API

        toast.success('Nitrogen Supervisor option updated successfully!')
    } catch (error) {
        console.error('Error updating Nitrogen Supervisor option:', error)
        toast.error('Failed to update Nitrogen Supervisor option.')
    }
}

const removeOption = async (index) => {
    try {
        await axios.delete(`${baseUrl}/api/job-tracker-master/nitrogen-supervisors/${listOptions.value[index].id}`)

        fetchAllNitrogenSupervisors() // Refresh the list options from the API

        toast.success('Nitrogen Supervisor option removed successfully!')
    } catch (error) {
        console.error('Error removing Nitrogen Supervisor option:', error)
        toast.error('Failed to remove Nitrogen Supervisor option.')
    }
}

const fetchAllNitrogenSupervisors = async () => {
    try {
        const response = await axios.get(`${baseUrl}/api/job-tracker-master/nitrogen-supervisors`)
        if (response.data && Array.isArray(response.data)) {
            listOptions.value = [] // Reset options before populating
            // Sort by nitrogen_supervisor_name alphabetically
            response.data.sort((a, b) => a.nitrogen_supervisor_name.localeCompare(b.nitrogen_supervisor_name))

            listOptions.value = response.data.map(nitrogen_supervisor => ({
                id: nitrogen_supervisor.id,
                value: nitrogen_supervisor.nitrogen_supervisor_name,
                label: nitrogen_supervisor.nitrogen_supervisor_name
            }))

            if (listOptions.value.length < 1 || !listOptions.value.some(opt => opt.value === selectedNitrogenSupervisor.value)) {
                selectedNitrogenSupervisor.value = ''
            }
        }
    } catch (error) {
        console.error('Error fetching Nitrogen Supervisor:', error)
    }
}

onMounted(async () => {
    await fetchAllNitrogenSupervisors()
})

watch(() => props.modelValue, (newValue) => {
    // Update selectedNitrogenSupervisor when modelValue changes
    selectedNitrogenSupervisor.value = newValue
})

watch(selectedNitrogenSupervisor, (newValue) => {
