    ...
    <div class="mb-5">
        <div class="flex justify-between items-center mb-2">
            ...
            <div class="flex items-center gap-2">
                <!-- reset button -->
                <button @click="selectedN2Converter = ''" type="button"
        ...
        <Listbox v-model="selectedN2Converter">
            <div class="relative">
                <ListboxButton
                    ...
                    <span class="block truncate text-gray-900 dark:text-white">
                        {{ selectedN2Converter || placeholder }}
                    </span>
                ...
                <transition leave-active-class="transition duration-100 ease-in" leave-from-class="opacity-100"
                    ...
                    <ListboxOptions
                        class="absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white dark:bg-gray-700 py-1 ring ring-gray-200 text-base shadow-lg ring-opacity-5 focus:outline-none sm:text-sm z-10">
        ...
        <div v-if="showAddOption" class="mt-2 flex gap-2">
            <input v-model="newOption" type="text" placeholder="Enter new N2 Converter"
...
const placeholder = ref('Select N2 Converter')
const listOptions = ref([])

const selectedN2Converter = ref('')
...
const addNewOption = async () => {
    if (newOption.value.trim()) {
        try {
            await axios.post(`${baseUrl}/api/job-tracker-master/n2-converters`, {
                n2_converter_name: newOption.value.trim()
            })

            fetchAllN2Converters() // Refresh the list options from the API

            toast.success('Control N2 Converter option added successfully!')

            newOption.value = ''
            showAddOption.value = false
        } catch (error) {
            console.error('Error adding N2 Converter option:', error)
            toast.error('Failed to add N2 Converter option.')
        }
    }
}
...
const updateOption = async ({ index, oldValue, newValue }) => {
    // Extract N2 Converter name from newValue object or use it directly if it's a string
    const newN2ConverterName = typeof newValue === 'object' ? newValue.n2_converter_name : newValue

    try {
        await axios.put(`${baseUrl}/api/job-tracker-master/n2-converters/${listOptions.value[index].id}`, {
            n2_converter_name: newN2ConverterName.trim()
        })

        fetchAllN2Converters() // Refresh the list options from the API

        toast.success('Control N2 Converter option updated successfully!')
    } catch (error) {
        console.error('Error updating N2 Converter option:', error)
        toast.error('Failed to update N2 Converter option.')
    }
}

const removeOption = async (index) => {
    try {
        await axios.delete(`${baseUrl}/api/job-tracker-master/n2-converters/${listOptions.value[index].id}`)

        fetchAllN2Converters() // Refresh the list options from the API

        toast.success('Control N2 Converter option removed successfully!')
    } catch (error) {
        console.error('Error removing N2 Converter option:', error)
        toast.error('Failed to remove N2 Converter option.')
    }
}

const fetchAllN2Converters = async () => {
    try {
        const response = await axios.get(`${baseUrl}/api/job-tracker-master/n2-converters`)
        if (response.data && Array.isArray(response.data)) {
            listOptions.value = [] // Reset options before populating
            // Sort by n2_converter_name alphabetically
            response.data.sort((a, b) => a.n2_converter_name.localeCompare(b.n2_converter_name))

            listOptions.value = response.data.map(loc => ({
                id: loc.id,
                value: loc.n2_converter_name,
                label: loc.n2_converter_name
            }))

            if (listOptions.value.length < 1 || !listOptions.value.some(opt => opt.value === selectedN2Converter.value)) {
                selectedN2Converter.value = ''
            }
        }
    } catch (error) {
