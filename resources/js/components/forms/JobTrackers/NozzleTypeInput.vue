    ...
    <div class="mb-5">
        <div class="flex justify-between items-center mb-2">
            ...
            <div class="flex items-center gap-2">
                <!-- reset button -->
                <button @click="selectedNozzleType = ''" type="button"
        ...
        <Listbox v-model="selectedNozzleType">
            <div class="relative">
                <ListboxButton
                    ...
                    <span class="block truncate text-gray-900 dark:text-white">
                        {{ selectedNozzleType || placeholder }}
                    </span>
        ...
        <div v-if="showAddOption" class="mt-2 flex gap-2">
            <input v-model="newOption" type="text" placeholder="Enter new nozzle type"
...
const placeholder = ref('Select nozzle type')
const listOptions = ref([])

const selectedNozzleType = ref('')
...
const addNewOption = async () => {
    if (newOption.value.trim()) {
        try {
            await axios.post(`${baseUrl}/api/job-tracker-master/nozzle-types`, {
                type_name: newOption.value.trim()
            })

            fetchAllNozzleTypes() // Refresh the list options from the API

            toast.success('Nozzle type option added successfully!')
...
const updateOption = async ({ index, oldValue, newValue }) => {
    ...
    const newNozzleTypeName = typeof newValue === 'object' ? newValue.type_name : newValue

    try {
        await axios.put(`${baseUrl}/api/job-tracker-master/nozzle-types/${listOptions.value[index].id}`, {
            type_name: newNozzleTypeName.trim()
        })

        fetchAllNozzleTypes() // Refresh the list options from the API

        toast.success('Nozzle type option updated successfully!')
    } catch (error) {
...
const removeOption = async (index) => {
    try {
        await axios.delete(`${baseUrl}/api/job-tracker-master/nozzle-types/${listOptions.value[index].id}`)

        fetchAllNozzleTypes() // Refresh the list options from the API

        toast.success('Nozzle type option removed successfully!')
    } catch (error) {
...
const fetchAllNozzleTypes = async () => {
    try {
        const response = await axios.get(`${baseUrl}/api/job-tracker-master/nozzle-types`)
        if (response.data && Array.isArray(response.data)) {
            ...
            if (listOptions.value.length < 1 || !listOptions.value.some(opt => opt.value === selectedNozzleType.value)) {
                selectedNozzleType.value = ''
            }
        }
    } catch (error) {
