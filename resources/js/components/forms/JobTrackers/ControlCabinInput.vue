    ...
    <div class="mb-5">
        <div class="flex justify-between items-center mb-2">
            ...
            <div class="flex items-center gap-2">
                <!-- reset button -->
                <button @click="selectedControlCabin = ''" type="button"
        ...
        <Listbox v-model="selectedControlCabin">
            <div class="relative">
                <ListboxButton
                    ...
                    <span class="block truncate text-gray-900 dark:text-white">
                        {{ selectedControlCabin || placeholder }}
                    </span>
        ...
        <div v-if="showAddOption" class="mt-2 flex gap-2">
            <input v-model="newOption" type="text" placeholder="Enter new control cabin"
...
const placeholder = ref('Select control cabin')
const listOptions = ref([])

const selectedControlCabin = ref('')
...
const addNewOption = async () => {
    if (newOption.value.trim()) {
        try {
            await axios.post(`${baseUrl}/api/job-tracker-master/control-cabins`, {
                cabin_name: newOption.value.trim()
            })

            fetchAllControlCabins() // Refresh the list options from the API
...
const updateOption = async ({ index, oldValue, newValue }) => {
    ...
    const newControlCabinName = typeof newValue === 'object' ? newValue.cabin_name : newValue

    try {
        await axios.put(`${baseUrl}/api/job-tracker-master/control-cabins/${listOptions.value[index].id}`, {
            cabin_name: newControlCabinName.trim()
        })
...
const removeOption = async (index) => {
    try {
        await axios.delete(`${baseUrl}/api/job-tracker-master/control-cabins/${listOptions.value[index].id}`)
...
const fetchAllControlCabins = async () => {
    try {
        const response = await axios.get(`${baseUrl}/api/job-tracker-master/control-cabins`)
        if (response.data && Array.isArray(response.data)) {
            listOptions.value = [] // Reset options before populating
            // Sort by cabin_name alphabetically
            response.data.sort((a, b) => a.cabin_name.localeCompare(b.cabin_name))

            listOptions.value = response.data.map(loc => ({
                id: loc.id,
                value: loc.cabin_name,
                label: loc.cabin_name
            }))

            if (listOptions.value.length < 1 || !listOptions.value.some(opt => opt.value === selectedControlCabin.value)) {
                selectedControlCabin.value = ''
            }
        }
    } catch (error) {
        console.error('Error fetching control cabin:', error)
    }
}

onMounted(async () => {
    await fetchAllControlCabins()
})

watch(() => props.modelValue, (newValue) => {
    // Update selectedControlCabin when modelValue changes
    selectedControlCabin.value = newValue
})

watch(selectedControlCabin, (newValue) => {
