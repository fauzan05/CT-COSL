    ...
    <div class="mb-5">
        <div class="flex justify-between items-center mb-2">
            ...
            <div class="flex items-center gap-2">
                <!-- reset button -->
                <button @click="selectedBOP = ''" type="button"
        ...
        <div v-if="showAddOption" class="mt-2 flex gap-2">
            <input v-model="newOption" type="text" placeholder="Enter new BOP"
...
const placeholder = ref('Select BOP')
const listOptions = ref([])

const selectedBOP = ref('')
...
const addNewOption = async () => {
    if (newOption.value.trim()) {
        try {
            await axios.post(`${baseUrl}/api/job-tracker-master/bops`, {
                bop_name: newOption.value.trim()
            })

            fetchAllBOPs() // Refresh the list options from the API

            toast.success('BOP option added successfully!')

            newOption.value = ''
            showAddOption.value = false
        } catch (error) {
            console.error('Error adding BOP option:', error)
            toast.error('Failed to add BOP option.')
        }
    }
}
...
const updateOption = async ({ index, oldValue, newValue }) => {
    // Extract BOP name from newValue object or use it directly if it's a string
    const newBOPName = typeof newValue === 'object' ? newValue.bop_name : newValue

    try {
        await axios.put(`${baseUrl}/api/job-tracker-master/bops/${listOptions.value[index].id}`, {
            bop_name: newBOPName.trim()
        })

        fetchAllBOPs() // Refresh the list options from the API

        toast.success('BOP option updated successfully!')
    } catch (error) {
        console.error('Error updating BOP option:', error)
        toast.error('Failed to update BOP option.')
    }
}

const removeOption = async (index) => {
    try {
        await axios.delete(`${baseUrl}/api/job-tracker-master/bops/${listOptions.value[index].id}`)

        fetchAllBOPs() // Refresh the list options from the API

        toast.success('BOP option removed successfully!')
    } catch (error) {
        console.error('Error removing BOP option:', error)
        toast.error('Failed to remove BOP option.')
    }
}

const fetchAllBOPs = async () => {
    try {
        const response = await axios.get(`${baseUrl}/api/job-tracker-master/bops`)
        if (response.data && Array.isArray(response.data)) {
            listOptions.value = [] // Reset options before populating
            // Sort by bop_name alphabetically
            response.data.sort((a, b) => a.bop_name.localeCompare(b.bop_name))
            ...
            if (listOptions.value.length < 1 || !listOptions.value.some(opt => opt.value === selectedBOP.value)) {
                selectedBOP.value = ''
            }
        }
    } catch (error) {
