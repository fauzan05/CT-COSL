    ...
    <div class="mb-5">
        <div class="flex justify-between items-center mb-2">
            ...
            <div class="flex items-center gap-2">
                <!-- reset button -->
                <button @click="selectedCTSupervisor = ''" type="button"
        ...
        <Listbox v-model="selectedCTSupervisor">
            <div class="relative">
                <ListboxButton
                    ...
                    <span class="block truncate text-gray-900 dark:text-white">
                        {{ selectedCTSupervisor || placeholder }}
                    </span>
        ...
        <div v-if="showAddOption" class="mt-2 flex gap-2">
            <input v-model="newOption" type="text" placeholder="Enter new CT Supervisor"
...
const placeholder = ref('Select CT Supervisor')
const listOptions = ref([])

const selectedCTSupervisor = ref('')
...
const addNewOption = async () => {
    if (newOption.value.trim()) {
        try {
            await axios.post(`${baseUrl}/api/job-tracker-master/ct-supervisors`, {
                ct_supervisor_name: newOption.value.trim()
            })

            fetchAllCTSupervisors() // Refresh the list options from the API

            toast.success('CT Supervisor option added successfully!')

            newOption.value = ''
            showAddOption.value = false
        } catch (error) {
            console.error('Error adding CT Supervisor option:', error)
            toast.error('Failed to add CT Supervisor option.')
        }
    }
}
...
const updateOption = async ({ index, oldValue, newValue }) => {
    // Extract CT Supervisor name from newValue object or use it directly if it's a string
    const newCTSupervisorName = typeof newValue === 'object' ? newValue.ct_supervisor_name : newValue

    try {
        await axios.put(`${baseUrl}/api/job-tracker-master/ct-supervisors/${listOptions.value[index].id}`, {
            ct_supervisor_name: newCTSupervisorName.trim()
        })

        fetchAllCTSupervisors() // Refresh the list options from the API

        toast.success('CT Supervisor option updated successfully!')
    } catch (error) {
        console.error('Error updating CT Supervisor option:', error)
        toast.error('Failed to update CT Supervisor option.')
    }
}

const removeOption = async (index) => {
    try {
        await axios.delete(`${baseUrl}/api/job-tracker-master/ct-supervisors/${listOptions.value[index].id}`)

        fetchAllCTSupervisors() // Refresh the list options from the API

        toast.success('CT Supervisor option removed successfully!')
    } catch (error) {
        console.error('Error removing CT Supervisor option:', error)
        toast.error('Failed to remove CT Supervisor option.')
    }
}

const fetchAllCTSupervisors = async () => {
    try {
        const response = await axios.get(`${baseUrl}/api/job-tracker-master/ct-supervisors`)
        if (response.data && Array.isArray(response.data)) {
            listOptions.value = [] // Reset options before populating
            // Sort by ct_supervisor_name alphabetically
            response.data.sort((a, b) => a.ct_supervisor_name.localeCompare(b.ct_supervisor_name))

            listOptions.value = response.data.map(ct_supervisor => ({
                id: ct_supervisor.id,
                value: ct_supervisor.ct_supervisor_name,
                label: ct_supervisor.ct_supervisor_name
            }))

            if (listOptions.value.length < 1 || !listOptions.value.some(opt => opt.value === selectedCTSupervisor.value)) {
                selectedCTSupervisor.value = ''
            }
        }
    } catch (error) {
