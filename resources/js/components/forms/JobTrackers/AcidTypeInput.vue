    ...
    <div>
        ...
        <div class="space-y-2 w-auto">
            <div v-for="(acid_type, index) in acidTypes" :key="`acid_type-${index}`" class="flex items-center gap-2 w-auto">
                <input v-model="acid_type.value" type="text"
                    ...
                    :placeholder="`Enter Acid Type ${index + 1}`" @input="handleInput(index, $event)" />

                <!-- Remove button (only show if more than 1 input) -->
                <button v-if="acidTypes.length > 1" @click="removeType(index)" type="button"
