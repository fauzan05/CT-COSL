    ...
    <div class="mb-5">
        ...
        <div class="grid grid-cols-3 gap-2">
            ...
            <div class="col-span-2">
                <Listbox v-model="selectedCTSize">
                    <div class="relative">
                        <ListboxButton
                            ...
                            <span class="block truncate text-gray-900 dark:text-white">
                                {{ selectedCTSize || sizePlaceholder }}
                            </span>
...
const completeSelection = computed(() => {
    if (selectedCTSize.value && selectedUnit.value) {
        return {
            size: selectedCTSize.value,
            unit: selectedUnit.value,
            label: `${selectedCTSize.value} ${selectedUnit.value}`
        }
    }
    return {
        size: 0,
...
const addNewOption = async () => {
    if (newOptionSize.value.trim()) {
        try {
            await axios.post(`${baseUrl}/api/job-tracker-master/ct-sizes`, {
                size: newOptionSize.value.trim()
            })

            await fetchAllCTSizes() // Refresh the list options from the API

            toast.success('CT Size option added successfully!')

            newOptionSize.value = ''
            showAddOption.value = false
        } catch (error) {
            console.error('Error adding CT Size option:', error)
            toast.error('Failed to add CT Size option.')
        }
    } else {
...
const updateOption = async ({ index, oldValue, newValue }) => {
    // Extract CT Size name from newValue object or use it directly if it's a string
    const newCTSizeName = typeof newValue === 'object' ? newValue.size : newValue

    try {
        await axios.put(`${baseUrl}/api/job-tracker-master/ct-sizes/${sizeOptions.value[index].id}`, {
            size: newCTSizeName.trim()
        })

        await fetchAllCTSizes() // Refresh the list options from the API

        toast.success('CT Size option updated successfully!')
    } catch (error) {
        console.error('Error updating CT Size option:', error)
        toast.error('Failed to update CT Size option.')
    }
}

const removeOption = async (index) => {
    try {
        await axios.delete(`${baseUrl}/api/job-tracker-master/ct-sizes/${sizeOptions.value[index].id}`)

        await fetchAllCTSizes() // Refresh the list options from the API

        toast.success('CT Size option removed successfully!')
    } catch (error) {
        console.error('Error removing CT Size option:', error)
        toast.error('Failed to remove CT Size option.')
    }
}

const fetchAllCTSizes = async () => {
    try {
