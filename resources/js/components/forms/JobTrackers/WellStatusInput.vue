...
const addNewOption = async () => {
    if (newOption.value.trim()) {
        try {
            const response = await axios.post(`${baseUrl}/api/job-tracker-master/well-statuses`, {
                status_name: newOption.value.trim()
            })

            fetchAllWellStatuss() // Refresh the list options from the API
...
const fetchAllWellStatuss = async () => {
    try {
