    ...
    <div class="mb-5">
        <div class="flex justify-between items-center mb-2">
            ...
            <div class="flex items-center gap-2">
                <!-- reset button -->
                <button @click="selectedCTString = ''" type="button"
        ...
        <Listbox v-model="selectedCTString">
            <div class="relative">
                <ListboxButton
        ...
        <div v-if="showAddOption" class="mt-2 flex gap-2">
            <input v-model="newOption" type="text" placeholder="Enter new CT String"
...
const placeholder = ref('Select CT String')
const listOptions = ref([])

const selectedCTString = ref('')
...
const addNewOption = async () => {
    if (newOption.value.trim()) {
        try {
            await axios.post(`${baseUrl}/api/job-tracker-master/ct-strings`, {
                ct_string_name: newOption.value.trim()
            })

            fetchAllCTStrings() // Refresh the list options from the API

            toast.success('Control CT String option added successfully!')

            newOption.value = ''
            showAddOption.value = false
        } catch (error) {
            console.error('Error adding CT String option:', error)
            toast.error('Failed to add CT String option.')
        }
    }
}
...
const updateOption = async ({ index, oldValue, newValue }) => {
    // Extract CT String name from newValue object or use it directly if it's a string
    const newCTStringName = typeof newValue === 'object' ? newValue.ct_string_name : newValue

    try {
        await axios.put(`${baseUrl}/api/job-tracker-master/ct-strings/${listOptions.value[index].id}`, {
            ct_string_name: newCTStringName.trim()
        })

        fetchAllCTStrings() // Refresh the list options from the API

        toast.success('Control CT String option updated successfully!')
    } catch (error) {
        console.error('Error updating CT String option:', error)
        toast.error('Failed to update CT String option.')
    }
}

const removeOption = async (index) => {
    try {
        await axios.delete(`${baseUrl}/api/job-tracker-master/ct-strings/${listOptions.value[index].id}`)

        fetchAllCTStrings() // Refresh the list options from the API

        toast.success('Control CT String option removed successfully!')
    } catch (error) {
        console.error('Error removing CT String option:', error)
        toast.error('Failed to remove CT String option.')
    }
}

const fetchAllCTStrings = async () => {
    try {
        const response = await axios.get(`${baseUrl}/api/job-tracker-master/ct-strings`)
        if (response.data && Array.isArray(response.data)) {
            listOptions.value = [] // Reset options before populating
            // Sort by ct_string_name alphabetically
            response.data.sort((a, b) => a.ct_string_name.localeCompare(b.ct_string_name))

            listOptions.value = response.data.map(loc => ({
                id: loc.id,
                value: loc.ct_string_name,
                label: loc.ct_string_name
            }))

            if (listOptions.value.length < 1 || !listOptions.value.some(opt => opt.value === selectedCTString.value)) {
                selectedCTString.value = ''
            }
        }
    } catch (error) {
        console.error('Error fetching CT String:', error)
    }
}

onMounted(async () => {
    await fetchAllCTStrings()
})

watch(() => props.modelValue, (newValue) => {
    // Update selectedCTString when modelValue changes
    selectedCTString.value = newValue
})

watch(selectedCTString, (newValue) => {
