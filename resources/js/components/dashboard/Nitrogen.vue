<template>
    <head>
        <Title>Nitrogen Documents</Title>
    </head>
                        ...
                        <DialogPanel
                            ...
                            <div class="mt-4 text-center space-y-4">
                                <p class="text-gray-700 dark:text-gray-200">
                                    ...
                                    <span class="font-semibold text-red-600">{{ selectedDocument?.type }}</span>?
                            ...
                            <div class="mt-6 flex justify-center space-x-3">
                                ...
                                <button type="button" :disabled="isDeleting" @click="handleDeleteDocument"
    ...
    <div class="p-6 bg-gray-50 min-h-screen dark:bg-slate-900/50 dark:text-gray-100 rounded-xl">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm">
            <div class="p-6 border-b border-gray-200 dark:border-gray-700">
                <h4 class="text-lg font-semibold text-gray-800 dark:text-white">Document List</h4>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and monitor your document inventory</p>
            </div>
                ...
                <div v-if="isLoading" class="overflow-x-auto">
                    <table class="min-w-full">
                        <thead class="bg-gray-50 dark:bg-gray-700/50">
                            <tr>
                                <th v-for="header in ['No', 'Name', 'Description', 'Document Total', 'Updated At', 'Updated By', 'Action']"
                                    :key="header"
...
const openDocumentModal = async (document) => {
    if (document) {
        ...
        documentForm.value.description = document.description;
        documentForm.value.documents = document.document_list;
...
const confirmDeleteModal = (document) => {
    selectedDocument.value = document;
...
async function fetchDocuments(page = 1) {
    try {
        ...
        const response = await axios.get(`${baseUrl}/api/documents?page=${page}&per_page=${perPage.value}&search=${search.value}&sort_by=${selectedSortByFilter.value.value}&sort_direction=${sortDirection.value ? 'desc' : 'asc'}&menu=nitrogen`, {
            headers: {
...
const handleDocumentDownload = (document) => {
    ...
    const url = `${baseUrl}/documents/${document.filename}`;
...
const handleDocumentSave = async () => {
    ...
    try {
        ...
        if (isCreate.value) {
            await axios.post(`${baseUrl}/api/documents`, formData, {
                headers: { 'Content-Type': 'multipart/form-data' }
            });
            useToast().success('Document created successfully!');
        } else {
            formData.append('_method', 'put');
            await axios.post(`${baseUrl}/api/documents/${selectedDocument.value.id}`, formData, {
                headers: { 'Content-Type': 'multipart/form-data' }
            });
...
function handleDeleteDocument() {
    ...
    let ids = [selectedDocument.value.id];
    axios.delete(`${baseUrl}/api/documents`, {
        data: { ids }
    })
        .then(response => {
