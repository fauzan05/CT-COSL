...
async function fetchDocuments(page = 1) {
    try {
        ...
        const response = await axios.get(`${baseUrl}/api/documents?page=${page}&per_page=${perPage.value}&search=${search.value}&sort_by=${selectedSortByFilter.value.value}&is_desc=${isDesc.value}`);
